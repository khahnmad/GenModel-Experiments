...
class TFG:
        ...
        'max_length': 100, # changed from 4, None doesn't work, 500 causes issues w vicuna models # TODO  
        'min_length': 1,
        'num_beam_groups': 2,
        'num_beams': 2,
        #'max_tokens': 32,
        ...
        def length_function(self, prompt):
                return self.tokenizer(prompt, return_tensors="pt").input_ids.real.shape[1]
        
        def split_text(self, text, n_template_tokens):
            chunk_size = self.tokenizer.model_max_length-n_template_tokens # 85 is the lenght of the prompt 

            text_splitter = RecursiveCharacterTextSplitter(chunk_size=chunk_size, length_function=self.length_function)
...
class _Tools(TFG):
            ...
            def _t5_loader(TFG):
                ...
                """
                TFG.tokenizer = T5Tokenizer.from_pretrained(TFG._model_name,use_auth_token=True)
                print(f'{TFG._model_name} - Tokenizer loaded')
                if TFG._connect_to_gpu:
                    TFG.model = T5ForConditionalGeneration.from_pretrained(TFG._model_name, device_map="auto", load_in_8bit = TFG._memory_saver,use_auth_token=True)
                else:
                    ...
                    TFG.model = T5ForConditionalGeneration.from_pretrained(TFG._model_name,use_auth_token=True,).to(TFG._device)
...
class _Generation(TFG):
            ...
            def _generation_with_score_and_split(TFG, prompt:dict)->tuple:
                if prompt['split_method'] == 'langchain':
                    n_template = len(prompt['template'])
                    prompts = TFG.split_text(prompt['prompt'][n_template:],prompt['n_template_tokens'])
                    a, s = [],[]
                    for p in prompts:
                        partial_prompt = prompt
                        partial_prompt['prompt'] = prompt['template']+str(p.page_content)
                        response = _Generation._generation_with_score(TFG, partial_prompt)
                        a.append(response[0])
                        s.append(response[1])
                    return a,s,True
                elif prompt['split_method']=='truncated':
                    
                    response = _Generation._generation_with_score(TFG, prompt, truncation=True)
                    return response[0], response[1],True
                else:
                    raise ValueError(f"split_method must be 'truncated' or 'langchain', not {split_method}")

                    

            def _generation_with_score(TFG, prompt :dict, truncation=False) -> tuple:
                ...
                """
                input_ids = TFG.tokenizer(prompt['prompt'], return_tensors="pt", truncation=truncation).input_ids.to(TFG._device)
                max_tokens = TFG.tokenizer.model_max_length
                prompt['n_template_tokens'] = TFG.tokenizer(prompt['template'], return_tensors="pt").input_ids.real.shape[1]
                
                if input_ids.real.shape[1] > max_tokens:
                    return _Generation._generation_with_score_and_split(TFG,prompt)
                                 
                output = TFG.model.generate(input_ids, **TFG._parameters,max_new_tokens =100) 
            ...
            def _generate_from_tfg(TFG, prompt :dict, text_only :bool) -> Union[dict, str]:
