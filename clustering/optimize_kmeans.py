...
def do_kmeans_clustering(n_clusters, embeddings)->tuple:
    ...
    return clustering, cluster_centers, float(silhouette_score)
...
def optimize_cluster_number(start_n:int, end_n:int, interval:int, embeddings:list):
    ...
    for n in range(start_n, end_n+1,interval):
        a = time.time()
        clustering, centers, score = do_kmeans_clustering(n, embeddings)
        b = time.time()
...
for hvv in ['hero','villain','victim']:
    ...
    sf.export_as_json(f'kmeans_n_optimization_{start_n}_{end_n}_{hvv}.json',{'content':optimization,
                                                                       'metadata':{'start':start_n,
                                                                                   'end':end_n,
                                                                                   'clustering':'kmeans',
                                                                                   'initial_subsample':True,
