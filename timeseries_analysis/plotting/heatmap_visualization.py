...
def generate_grid(i_level, hvv, length, bin_size):
    grid = []
...
def plot_heatmap(level, he_vi_vic, segment_length, binsize):
    data = generate_grid(level, he_vi_vic, segment_length, binsize)
...
for input_level in inputs.keys():
    for hvv in inputs[input_level]:
        plot_heatmap(input_level, hvv, segment_length='full', binsize='month')
