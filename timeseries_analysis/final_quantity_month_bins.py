...
def apply_binning(elts: list) -> list:
    ...
    for elt in elts:
        ...
        doc = {'hero': elt[0],
               ...
               'time': datetime.date(year=year, month=month,day=1),
               'partisanship': elt[4],
...
def fetch_all_signals(data, h_v_v, partisanships, input_level):
    ...
    try: # look for existing file
        output = sf.import_json(f'month_signals/{partisanships[0]}_signals_by_part_input_hvv.json')
    ...
    filename = f'month_signals/{partisanships[0]}_signals_by_part_input_hvv.json'
