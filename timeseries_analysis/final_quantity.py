"""
Final answer for: 
How many narratives move from FR to Center partisanships with a correlation of 0.75 or greater in the {1,2}
displacement period 
"""
...
def fetch_all_characters(partisan_b, input_level,h_v_v)->list:
    partisan_a = 'FarRight'
    ...
    characters = [x[0] for x in counter if x[1]>=5 and x[0]!='none' and x[0]!='time']
    return characters
...
def fetch_signal(character:str, hvv:str, partisanships, input_level)->list:
    ...
    for file in pool_alphabet_files: 
        ...
        if input_level=='single':
            ...
            content += [x for x in data if f"{x[hvv_indexing[hvv[0]]]}.{x[hvv_indexing[hvv[1]]]}".lower()==character and x[4] in partisanships]
...
def fetch_all_signals(data, h_v_v, partisanships, input_level):
    ...
    try:
        output = sf.import_json(f'{partisanships[0]}_signals_by_part_input_hvv.json')
    ...
    filename = f'{partisanships[0]}_signals_by_part_input_hvv.json'
...
for input_level in inputs.keys():
    for hvv in inputs[input_level]:
        for part_b in ['CenterLeft','Center','CenterRight']:
            print(f"For {input_level}, {hvv}, {part_b}:")
            characters = fetch_all_characters(part_b, h_v_v=hvv, input_level=input_level)
            ...
            if input_level=='tuple':
                sf.export_nested_list(f"{part_a}_{input_level}_{'-'.join(hvv)}_{part_b}.csv",high_correlation)
            else:
                sf.export_nested_list(f"{part_a}_{input_level}_{hvv}_{part_b}.csv",high_correlation)
