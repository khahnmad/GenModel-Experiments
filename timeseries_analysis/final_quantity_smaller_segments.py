...
def find_fr_influence_narratives(signals, segment_length):
    ...
    for signal_pair in signals:
        ...
        for i in range(len(signal_pair[1])):
            # First segment (3months): 0:6,1:7,2:8
            # Generate correlation
            start =i
            stop = i+segment_length
            signal_a = signal_pair[1][i:i+segment_length]
            signal_b = signal_pair[2][i:i+segment_length]
            if len(signal_a)<segment_length:
                continue
            x, y = [], []
            for l in [1, 2]:

                r, p = scipy.stats.pearsonr(signal_a[:-l], signal_b[l:])  # coefficient, p-value

                x.append(l)
                y.append(r)
            if y[0] >= threshold or y[1] >= threshold:
                high_correlation.append([signal_pair[0], y, start, stop])
